...
//   async (userId: string, { rejectWithValue }) => {
//     try {
//       const response = await apiService.checkFavorite(userId);
...
//       state.favorites = state.favorites.filter(user => user._id !== action.payload);
//       state.pagination.totalFavorites = Math.max(0, state.pagination.totalFavorites - 1);
//     },
//     addFavorite: (state, action: PayloadAction<FavoriteUser>) => {
//       // Prevent duplicates
//       if (!state.favorites.find(user => user._id === action.payload._id)) {
...
//             const existingIds = new Set(state.favorites.map(user => user._id));
//             const newFavorites = favorites.filter(user => !existingIds.has(user._id));
...
//             state.favorites = state.favorites.filter(user => user._id !== userId);
//             state.pagination.totalFavorites = Math.max(0, state.pagination.totalFavorites - 1);
//           }
//           // For 'added', we don't add to list immediately - user should refetch or we'd need user data
//         }
//       })
//       // Check Favorite
//       .addCase(checkFavorite.fulfilled, (state, action) => {
//         // You can store checked status in state if needed
//         // const { userId, isFavorite } = action.payload;
//       });
//   },
// });
